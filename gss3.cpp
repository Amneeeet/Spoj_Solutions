...
node query(int index,int start,int end,int l,int r)
{
    ...
    int mid = (start+end)/2;
    if (l > mid)// properly in the right subtree
        return query(2*index+2, mid+1, end, l, r);
    if (r <= mid)// properly in the left subtree
...
///***************************************************************************
void update(int index, int lo, int hi, int st, int value) 
{
  ...
  // index is updated now update all the values
